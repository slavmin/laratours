...
class GibddNotifyRepository
{
    ...
    public function getTourAddresses($tour_id)
    {
        ...
        $tour_days_count = $this->tour->days;
        $tour_nights_count = $this->tour->nights;
        ...
        $needed_types = [
            "App\Models\Tour\TourMuseum",
            "App\Models\Tour\TourMeal",
            "App\Models\Tour\TourHotel",
        ];

        foreach ($this->tour->object_attributes as $object) {
            if (in_array($object->objectable_type, $needed_types)) {
                $object_properties = TourObjectAttributeProperties::where('tour_id', $this->tour->id)
                    ->where('object_attribute_id', $object->id)->first();
                $object_days = json_decode($object_properties->days_array);

                foreach ($object_days as $object_day) {
                    if (in_array($object_day, $transport_days)) {
                        $object_parent = $object->objectable_type::find($object->objectable_id);
                        $object_data = [
                            'name'  => $object_parent->name,
                            'event' => $object->name,
                        ];

                        $object_address = $object_parent->address;
                        if ($object_address != null) {
                            $object_data['address'] = $object_address;
                        } else {
                            $object_data['address'] = 'Адрес не указан';
                        }

                        array_push($result[$object_day], $object_data);
                    }
                }
            }
        }
    ...
    public function getTourTransportDays($tour_id)
    {
        ...
        $tour_objects = $this->tour->object_attributes;
        foreach ($tour_objects as $obj) {
            if ($obj->objectable_type == 'App\Models\Tour\TourTransport') {
                $this->tour_transport[] = $obj;
            }
        }

        $result = [];

        foreach ($this->tour_transport as $transport) {
            $transport_properties = TourObjectAttributeProperties::where('tour_id', $this->tour->id)
                ->where('object_type', 'transport')
                ->where('object_attribute_id', $transport->id)
                ->first();

            $transport_days = json_decode($transport_properties->days_array);
    ...
    public function getDataForGibddNotify($tour_id, $tour_day)
    {
        ...
        $tour_date =  $this->tour->dates[0]->date;
        $form1 = [
            ...
            'goal' => '',
            'dateStart' => substr($tour_date, 0, 10),
            'timeStart' => substr($tour_date, 11),
            'dateEnd' => '',
            'timeEnd' => '',
        ];
        ...
        $tour_transport_company = null;
        $tour_transport_bus = null;

        foreach ($this->tour->object_attributes as $object) {
            $object_properties = TourObjectAttributeProperties::where('tour_id', $this->tour->id)
                ->where('object_attribute_id', $object->id)
                ->where('object_type', 'transport')
                ->first();

            if ($object_properties != null && in_array($tour_day, json_decode($object_properties->days_array))) {
                $tour_transport_bus = $object;
                $tour_transport_company = TourTransport::findOrFail($object->objectable_id);
        ...
        $form3 = [
            ...
            'name'              => '',
            'entity_address'    => $tour_transport_company->address ?? '',
            'address'           => '',
            'entity_location'   => $tour_transport_company->address ?? '',
            'phone'             => $tour_transport_company->phone ?? '',
            'email'             => $tour_transport_company->email ?? '',
            'inn'               => $tour_transport_company->inn ??  '',
        ];

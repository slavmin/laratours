...
<v-container fluid grid-list-md text-xs-center>
  ...
  <v-row justify="center">
    <v-btn fab dark class="tc-red-bg tc-link-no-underline-on-hover" title="@lang('buttons.general.crud.create')"
      href="{{ route('frontend.tour.attendant.create') }}">
  ...
  <v-row>
    <v-col>
      <v-card>
        <v-toolbar prominent dark color="#94CED7" src="/img/frontend/attendant_tmpl.jpg"></v-toolbar>
        @if (count($items)>0)
        <v-simple-table class="mt-5">
          <template v-slot:default>
            <thead>
              <tr>
                <th class="text-left">
                  @lang('labels.frontend.tours.attendant.table.name')
                </th>
                <th class="text-left">
                  @lang('labels.frontend.tours.attendant.table.description')
                </th>
                <th class="text-right">

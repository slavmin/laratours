      ...
      <v-card>
        ...
        <v-card-text id="order">
          ...
          <v-row>
            <v-col lg="4" cols="12">
              <v-text-field name="contact_name" color="#aa282a"
                label="{{ __('labels.frontend.user.profile.first_name') }}" value="{{ $item->contact_name ?? '' }}"
                maxlength="191" outlined required></v-text-field>
            </v-col>
            <v-col lg="4" cols="12">
              <v-text-field name="email" prepend-icon="email" color="#aa282a"
                label="{{ __('labels.frontend.user.profile.email') }}" value="{{ $item->email ?? '' }}" maxlength="191"
                outlined required></v-text-field>
            </v-col>
            <v-col lg="4" cols="12">
              <v-text-field name="phone" prepend-icon="phone" color="#aa282a"
                label="{{ __('labels.frontend.user.profile.phone') }}" value="{{ $item->phone ?? '' }}" maxlength="191"
                outlined required></v-text-field>
            </v-col>
          </v-row>
          <h3 class="text-grey text-center">
          ...
          <order-form private-form :old="{{ json_encode($item->profiles[0]->content) }}"
            :transport="{{ json_encode($tour_transport) }}" :profiles="{{ json_encode($tour->orderprofiles) }}"
